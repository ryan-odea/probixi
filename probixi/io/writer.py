...
# Per-reflection columns appended after ``panel``; CrystFEL ignores trailing fields.
EXTRA_REFLECTION_COLUMNS = ("n_pix", "bg_model", "bg_model_var")
...
def _integration_lines(recipe: Optional[dict]) -> list[str]:
    # Header key/values a merging program reads back; CrystFEL skips unknown lines
    if not recipe:
        return []
    out = []
    radii = recipe.get("radii")
    if radii:
        out.append("probixi/int_radius = {:.2f},{:.2f},{:.2f}".format(*radii))
    gain = recipe.get("adu_per_photon")
    if gain:
        out.append(f"probixi/adu_per_photon = {float(gain):.6g}")
    n_bg = recipe.get("bg_annulus_pixels")
    if n_bg:
        out.append(f"probixi/bg_annulus_pixels = {int(n_bg)}")
    if recipe.get("aperture"):
        out.append(f"probixi/aperture = {recipe['aperture']}")
    out.append(f"probixi/reflection_columns = {' '.join(EXTRA_REFLECTION_COLUMNS)}")
    return out


def _reflection_extras(result) -> tuple:
    # (n_pix, bg_model, bg_model_var) host lists, or () when not integrated
    cols = (
        result.predicted_n_pixels,
        result.predicted_bg_model,
        result.predicted_bg_model_var,
    )
    if any(c is None for c in cols):
        return ()
    return tuple(c.detach().cpu().tolist() for c in cols)
...
class _StreamWriter:
    ...
    def _write_header(self) -> None:
        ...
        lines += _integration_lines(self.integration)
...
class DataOffloader(_StreamWriter):
    ...
    integration : dict, optional
        ...
        ``bg_annulus_pixels``, ``aperture``) written as ``probixi/...`` header
        lines so a merging program can recover it; see
        :attr:`~probixi.Probixi.integration_recipe`.
    """
    ...
    def _format_crystal(
        ...
        if result.predicted_hkl is not None:
            ...
            refl = list(zip(p_pos, p_hkl, p_int, p_sig, p_pk, p_bg, *extras))
        else:
            extras = ()
            refl = [
                (rc, hk, i, s, 0.0, 0.0)
        ...
        fall = getattr(result, "falloff_limit", None)
        if fall is not None and math.isfinite(fall):
            lines.append(f"probixi/falloff_limit = {fall:.6f} nm^-1")
        lines += [
            ...
            "fs/px  ss/px panel"
            + ("  n_pix   bg_model bg_model_var" if extras else ""),
        ]
        for (
            (row, col),
            (h, k, l),  # noqa: E741
            ...
            *extra,
            ...
            if extra:
                n_pix, bg_model, bg_var = extra
                line += f" {int(n_pix):5d} {bg_model:10.2f} {bg_var:12.5g}"
            lines.append(line)
