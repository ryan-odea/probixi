...
class PeakOffloader(_StreamWriter):
    """Write peak-search results to a peaks-only CrystFEL ``.stream`` file.

    The peak-finding twin of :class:`DataOffloader`: each frame becomes a chunk
    carrying only the ``Peaks from peak search`` list (no crystal block,
    ``indexed_by = none``), so a downstream indexer of your choice can read the
    peaks back (e.g. ``indexamajig`` with ``--peaks=msgpack``/a re-fed stream).
    Use as a context manager; the instance is itself the per-frame writer
    expected by ``PeakStream.for_each`` (``__call__`` forwards to ``write``)::

        with PeakOffloader(out, geometry=geom, files=files) as off:
            pipeline.peak_stream(frames, estimate_scale=False).for_each(off)
    ...
    path : str or Path
        Destination stream file.
    geometry : dict
        Detector geometry (``beam_center``, ``clen``, ``pixel_size``,
        ``wavelength``, and -- when available -- the parsed ``panels``).
    geometry_file : str or Path, optional
        Geometry file embedded verbatim in the stream header when present.
    files : dict, optional
    ...
    def write(self, result: "PeakResult") -> None:
        """Append one frame's detected peaks to the stream as a CrystFEL chunk."""
        if self._fh is None:
            raise RuntimeError("PeakOffloader must be used as a context manager")
        self._serial += 1
        self._fh.write(self._format_chunk(result))

    def _format_chunk(self, result: "PeakResult") -> str:
        ...
        out = self._peak_search_lines(
            result.frame_index, positions, intensities, len(rows), "none"
        )
        out.append("----- End chunk -----")
        return "\n".join(out) + "\n"
