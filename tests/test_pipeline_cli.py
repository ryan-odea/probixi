...
def test_cli_indexing_run_writes_nonempty_stream(tmp_path, run_files, geom_path):
    ...
    assert "indexed_by = probixi" in text


def test_cli_peaks_only_without_cell_writes_peaks_stream(
    tmp_path, run_files, geom_path
):
    lst_path, _, _ = run_files
    ...
    text = out.read_text()
    assert len(text) > 0
    assert "Peaks from peak search" in text
    assert "indexed_by = none" in text
    # peaks-only emits no crystal solution block
    assert "--- Begin crystal" not in text
